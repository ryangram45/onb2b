 ...
 export default function ReviewDetailsClient() {
   ...
   const nickName = useMemo(() => searchParams.get("nickName") || "", [searchParams]);
   const recipientAddress = useMemo(() => searchParams.get("recipientAddress") || "", [searchParams]);
     ...
     <div className="mx-auto max-w-screen-md px-4 pb-28 pt-20">
       <div className="bg-white rounded-lg p-5 sm:p-6">
         ...
         <div className="space-y-4">
           <div>
             <h3 className="text-[0.95rem] font-semibold text-gray-900">Recipient Details</h3>
             <div className="mt-2 divide-y divide-gray-200">
               ...
               <div className="flex items-center justify-between py-2">
                 ...
                 <span className="text-[0.95rem] font-medium text-gray-900">{recipientFullName || "—"}</span>
               </div>
               <div className="flex items-center justify-between py-2">
                 <span className="text-[0.95rem] text-gray-700">Nickname (optional)</span>
                 <span className="text-[0.95rem] font-medium text-gray-900">{nickName || "—"}</span>
               </div>
               <div className="flex items-center justify-between py-2">
                 <span className="text-[0.95rem] text-gray-700">Recipient address</span>
                 <span className="text-[0.95rem] font-medium text-gray-900">{recipientAddress || "—"}</span>
               </div>
               <div className="flex items-center justify-between py-2">
                 <span className="text-[0.95rem] text-gray-700">City</span>
                 <span className="text-[0.95rem] font-medium text-gray-900">{city || "—"}</span>
               </div>
               <div className="flex items-center justify-between py-2">
                 <span className="text-[0.95rem] text-gray-700">State</span>
                 <span className="text-[0.95rem] font-medium text-gray-900">{state || "—"}</span>
               </div>
               <div className="flex items-center justify-between py-2">
                 <span className="text-[0.95rem] text-gray-700">ZIP code</span>
                 <span className="text-[0.95rem] font-medium text-gray-900">{zipPostalCode || "—"}</span>
               </div>
             </div>
           </div>
 
           <div>
             <h3 className="text-[0.95rem] font-semibold text-gray-900">Bank Details</h3>
             <div className="mt-2 divide-y divide-gray-200">
               {country === "United States" ? (
                 <>
                   <div className="flex items-center justify-between py-2">
                     <span className="text-[0.95rem] text-gray-700">Routing number</span>
                     <span className="text-[0.95rem] font-medium text-gray-900">{routingNumber || "—"}</span>
                   </div>
                   <div className="flex items-center justify-between py-2">
                     <span className="text-[0.95rem] text-gray-700">Account number</span>
                     <span className="text-[0.95rem] font-medium text-gray-900">{maskedAccount}</span>
                   </div>
                 </>
               ) : (
                 <>
                   <div className="flex items-center justify-between py-2">
                     <span className="text-[0.95rem] text-gray-700">SWIFT/BIC code</span>
                     <span className="text-[0.95rem] font-medium text-gray-900">{swiftBic || "—"}</span>
                   </div>
                   <div className="flex items-center justify-between py-2">
                     <span className="text-[0.95rem] text-gray-700">Account number</span>
                     <span className="text-[0.95rem] font-medium text-gray-900">{maskedAccount}</span>
         ...
         <div className="mt-6 grid place-items-center">
           <Button variant="outline" onClick={handleEditClick} className="rounded-full px-6 cursor-pointer">
             EDIT
           </Button>
         </div>
       </div>
