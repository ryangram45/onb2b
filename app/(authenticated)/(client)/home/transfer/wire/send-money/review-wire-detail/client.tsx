...
export default function ReviewWireDetailClient() {
  ...
  const [accountLabel, setAccountLabel] = useState<string>(accountLabelFromQuery);
  const [authorizationAccepted, setAuthorizationAccepted] = useState<boolean>(false);
      ...
      try {
        if (recipientId) {
          ...
          if (res.ok) {
            ...
            const foundRecipient = recipientList.find((candidate) => candidate.id === recipientId) || null;
        ...
        if (!accountLabel && accountId) {
          ...
          if (accountsResponse.ok) {
            ...
            const retrievedAccount = (accountsPayload?.accounts || []).find((acc: any) => acc.id === accountId);
            if (mounted && retrievedAccount) setAccountLabel(retrievedAccount.product === "AdvPlus Banking" ? "My Checking" : "My Saving");
          }
        }
      } catch {}
    })();
  ...
  const reqDate = today.toLocaleString("en-US", { month: "short", day: "numeric" });
  const totalCost = amountNumber + 30;

  const fullName = recipient ? `${recipient.firstName} ${recipient.lastName}`.trim() : "";
  const addressLines = [
  ...
  const handleSendButtonClick = () => {
    ...
    router.push(`/home/transfer/wire/send-money/otp?${params.toString()}`);
  };

  return (
    <div className="mx-auto max-w-screen-md px-4 pb-28 pt-20">
      <div className="bg-white rounded-lg p-5 sm:p-6">
        <h2 className="text-xl font-semibold text-gray-900 tracking-wide">Review wire details</h2>

        <InfoNote>
          <p>Any wire sent after 5 p.m. ET will be processed the next business day.</p>
        </InfoNote>

        <div className="mt-4 border-y border-gray-200 divide-y divide-gray-200">
          <div className="flex items-center justify-between py-3">
            <span className="text-[0.95rem] text-gray-700">Transfer request date</span>
          ...
          <div className="flex items-center justify-between py-3">
            <span className="text-[0.95rem] text-gray-700 block mb-1">Recipient address</span>
        ...
        <div className="mt-6">
          <h3 className="text-[0.95rem] font-semibold text-gray-900">Your total transfer cost</h3>
          <div className="mt-2">
            <div className="flex items-center justify-between py-3 border-b border-gray-200">
              <span className="text-[0.95rem] text-gray-700">Transfer amount</span>
              <span className="flex flex-col items-end">
                <span className="text-onb2b-blue-950 font-medium">{formatCurrency(amountNumber)} USD</span>
                {accountLabel && <span className="text-xs text-gray-600">{accountLabel}</span>}
              </span>
            </div>
            <div className="flex items-center justify-between py-3 border-b border-gray-200">
            ...
            <div className="flex items-center justify-between py-3">
              ...
              <span className="text-gray-900 font-semibold">{formatCurrency(totalCost)} USD</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 mx-auto border-t border-gray-200 flex items-center justify-between py-2 px-4 sm:px-6 max-w-screen-md z-50 bg-white/50">
        <Button variant="whiteGhost" onClick={() => router.back()} className="flex-1 rounded-full text-onb2b-blue-950 no-underline bg-white font-semibold tracking-wide cursor-pointer">
          CANCEL
        </Button>
        <Button
