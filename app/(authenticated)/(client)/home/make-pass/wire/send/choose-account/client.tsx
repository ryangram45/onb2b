...
export default function ChooseAccountClient() {
  ...
  const onAmountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const wholeWithCommas = (wholeRaw || "0").replace(
      /\B(?=(\d{3})+(?!\d))/g,
      ",",
    );
    const formatted =
  ...
  const handleCancelClick = () => {
    router.push("/home/make-pass/wire/send");
  };
  ...
  const handleNextClick = () => {
    ...
    if (phase === "summary") {
      if (!selectedAccountId || numericAmount <= 0 || !additionalInfo.trim())
        return;
      ...
      if (selectedAccount?.label)
        params.set("accountLabel", selectedAccount.label);
      params.set("note", additionalInfo.trim());
      router.push(
        `/home/make-pass/wire/send/review-user-detail?${params.toString()}`,
      );
      return;
    }
  };
          ...
          <>
            ...
            {phase === "amount" && (
              <div className="mt-8">
                ...
                <InfoNote>
                  <p>
                    Wire limits: You can send higher dollar amounts when
                    enrolled in Secured Transfer
                  </p>
                </InfoNote>
              </div>
            )}
                ...
                <div className="mt-6">
                  ...
                  <div className="mt-2 border-y border-gray-200 py-2">
                    <p className="text-xs text-gray-600">
                      Instructions for recipient bank (optional)
                    </p>
                    <div className="">
                      <Input
